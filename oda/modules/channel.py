...
# play
@Client.on_message(
    command(["cplay", f"cplay@{BOT_USERNAME}"])
    & filters.group
    & ~filters.edited
    & ~filters.forwarded
    & ~filters.via_bot
...
async def play(_, message: Message):
    ...
    c = await app.get_chat_member(chid, BOT_ID)
    ...
    try:
        b = await app.get_chat_member(chid, ASSID)
        if b.status == "kicked":
            await message.reply_text(
                f"🔴 {ASSNAME} (@{ASSUSERNAME}) is banned in your chat **{conchat.linked_chat.title}**\n\nUnban it first to use music"
            )
            return
    ...
    if await is_active_chat(chid):
        position = await queues.put(chid, file=file_path)
